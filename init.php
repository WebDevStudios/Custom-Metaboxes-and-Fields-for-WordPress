...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			$meta = apply_filters( 'cmb_get_post_meta', get_post_meta( $post->ID, $field['id'], 'multicheck' != $field['type'] /* If multicheck this can be multiple values */ ), $field );

			echo '<tr>';

			if ( $field['type'] == "title" || $field['show_names'] == false ) {
				echo '<td colspan="2">';
			} else {
